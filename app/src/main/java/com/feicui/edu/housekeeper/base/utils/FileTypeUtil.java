...
public class FileTypeUtil {
	...
	//判断当前文件属于哪种类型
	public static String[] getFileTypeAndIconName(File file){
		...
		for (int x = 0; x < ICON_TYPE_Table.length; x++) {
			if (dot.equals(ICON_TYPE_Table[x][0])){
				str[0] = ICON_TYPE_Table[x][2];
				str[1] = ICON_TYPE_Table[x][1];
				return str;
			}
		}
		return str;
	...
	public static String getMimeTypeFromFileName(File file){
		...
		//循环数组，进行后缀名之间的匹配
		for (int x = 0; x < MIME_Table.length; x++) {
			if (dot.equals(MIME_Table[x][0])){

				return MIME_Table[x][1];
			}
		}
		return "*/*";
	}
